...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        # Compute l(k, k+N)
        l_k_kN = -torch.log(torch.exp(sim(z_k, z_k_N) / tau) / 
                           torch.sum(torch.exp(torch.tensor([sim(z_k, out[j]) for j in range(2*N) if j != k]) / tau)))
        
        # Compute l(k+N, k)  
        l_kN_k = -torch.log(torch.exp(sim(z_k_N, z_k) / tau) / 
                           torch.sum(torch.exp(torch.tensor([sim(z_k_N, out[j]) for j in range(2*N) if j != k+N]) / tau)))
...
def compute_sim_matrix(out):
    ...
    # Compute similarity matrix: out_norm @ out_norm.T
