...
def train(model, data_loader, train_optimizer, epoch, epochs, batch_size=32, temperature=0.5, device='cuda'):
    ...
    for data_pair in train_bar:
        ...
        ##############################################################################
        # Get features and projections from the model
...
def train_val(model, data_loader, train_optimizer, epoch, epochs, device='cuda'):
    ...
    with (torch.enable_grad() if is_train else torch.no_grad()):
        for data, target in data_bar:
            ...
            # Check if model returns tuple (SimCLR) or single output (Classifier)
            model_output = model(data)
            if isinstance(model_output, tuple):
                feature, out = model_output
            else:
                out = model_output
    ...
    with torch.no_grad():
        ...
        for data, _, target in tqdm(memory_data_loader, desc='Feature extracting'):
            ...
            if isinstance(model_output, tuple):
                ...
                # For Classifier, we need to extract features from the encoder
                x = model.f(data.to(device))
