...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        caption_emb = self.embedding(captions)  # (N, T, wordvec_dim)
        caption_emb = self.positional_encoding(caption_emb)  # (N, T, wordvec_dim)
        
        # 2. Project image features to same dimension
        image_emb = self.visual_projection(features)  # (N, wordvec_dim)
        image_emb = image_emb.unsqueeze(1)  # (N, 1, wordvec_dim)
        
        # 3. Create target mask for future timesteps
        tgt_mask = torch.tril(torch.ones(T, T))  # (T, T)
        
        # 4. Apply transformer decoder
        decoder_output = self.transformer(caption_emb, image_emb, tgt_mask)  # (N, T, wordvec_dim)
        
        # 5. Project to vocabulary scores
        scores = self.output(decoder_output)  # (N, T, vocab_size)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        # 1. Convert input image to patch embeddings
        ...
        # 4. Average pool patch vectors to get feature vector for each image
