...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.1):
        ...
        # Initialize weights with specific values for testing
        torch.manual_seed(231)
        nn.init.normal_(self.query.weight, 0, 0.1)
        nn.init.normal_(self.key.weight, 0, 0.1)
        nn.init.normal_(self.value.weight, 0, 0.1)
        nn.init.normal_(self.proj.weight, 0, 0.1)
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # Reshape for multi-head attention: (N, S, E) -> (N, S, H, E/H)
        ...
        out = torch.matmul(attn_weights, V)  # (N, H, S, E/H)
        
        # Concatenate heads: (N, H, S, E/H) -> (N, S, E)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        # Reshape to patches: (N, C, H, W) -> (N, num_patches, patch_dim)
        patches = x.unfold(2, self.patch_size, self.patch_size).unfold(3, self.patch_size, self.patch_size)
        patches = patches.contiguous().view(N, self.num_patches, self.patch_dim)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src, src_mask=None):
        ...
        Inputs:
        - src: the sequence to the encoder, of shape (N, S, D)
        - src_mask: the parts of the source sequence to mask, of shape (S, S)
        
        Returns:
        - output: the Transformer features, of shape (N, S, D)
        """
        output = src
