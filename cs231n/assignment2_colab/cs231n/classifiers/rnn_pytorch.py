...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # (2) Word embeddings
        word_embeddings = word_embedding_forward(captions_in, W_embed)
        
        # (3) RNN/LSTM forward pass
        if self.cell_type == 'rnn':
            h = rnn_forward(word_embeddings, h0, Wx, Wh, b)
        else:  # LSTM
            h = lstm_forward(word_embeddings, h0, Wx, Wh, b)
        
        # (4) Temporal affine transformation to vocabulary scores
        vocab_scores = temporal_affine_forward(h, W_vocab, b_vocab)
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(max_length):
            ...
            # (3) Affine transformation to vocabulary scores
            vocab_scores = h @ W_vocab + b_vocab
            
            # (4) Select word with highest score
            prev_word = torch.argmax(vocab_scores, dim=1)
