...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            ...
            # Batch normalization parameters (if using batch norm)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            layer_output, cache = affine_forward(layer_input, W, b)
            caches.append(cache)
            
            # Batch normalization (if using)
            if self.normalization == 'batchnorm':
                gamma = self.params['gamma' + str(i + 1)]
                beta = self.params['beta' + str(i + 1)]
                layer_output, cache = batchnorm_forward(layer_output, gamma, beta, self.bn_params[i])
                caches.append(cache)
            
            # ReLU activation
            layer_output, cache = relu_forward(layer_output)
            caches.append(cache)
        ...
        scores, cache = affine_forward(layer_input, W_final, b_final)
        caches.append(cache)
        ...
        for i in range(self.num_layers - 1, 0, -1):
            ...
            # Batch normalization backward (if used)
            if self.normalization == 'batchnorm':
                cache = caches.pop()
